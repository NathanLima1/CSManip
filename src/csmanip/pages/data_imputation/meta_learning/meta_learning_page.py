...
class MetaLearningPage(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # --- Variáveis de Controle (Global Test) ---
        ...
        # --- Conteúdo dentro do scrollable_frame_left ---
        ...
        ttk.Combobox(custom_test_frame, values=lista_ml0, textvariable=self.ml_lv0_var, state='readonly').pack(fill=tk.X, padx=5, pady=(0, 10))
        ...
        ttk.Combobox(custom_test_frame, values=lista_tr0, textvariable=self.ml_tr0_var, state='readonly').pack(fill=tk.X, padx=5, pady=(0, 10))
        ...
        ttk.Combobox(custom_test_frame, values=lista_ml1, textvariable=self.ml_lv1_var, state='readonly').pack(fill=tk.X, padx=5, pady=(0, 10))
        ...
        ttk.Combobox(custom_test_frame, values=lista_ind_meta_p, textvariable=self.ind_meta_perso_var, state='readonly').pack(fill=tk.X, padx=5, pady=(0, 10))

        self.lbl_num_tests_ct = ttk.Label(custom_test_frame, text="")
        self.lbl_num_tests_ct.pack(anchor="w", padx=5)
        ttk.Entry(custom_test_frame, textvariable=self.num_teste_mtp_var, justify=tk.CENTER).pack(fill=tk.X, padx=5, pady=(0, 10))
        ...
        lista_type_input = ['Yes', 'No'] # Usar Yes/No aqui? Ou True/False com BooleanVar?
        ttk.Combobox(custom_test_frame, values=lista_type_input, textvariable=self.type_input_var, state='readonly').pack(fill=tk.X, padx=5, pady=(0, 10))
        ...
        # Cria checkboxes em loop para melhor organização
        self.ml_check_vars = { # Dicionário para guardar as variáveis
        ...
        self.ml_checkbuttons = {} # Dicionário para guardar os checkbuttons
        ...
        ttk.Combobox(global_test_frame, values=lista_ind_meta_p, textvariable=self.ind_meta_comb_var, state='readonly').pack(fill=tk.X, padx=5, pady=(0, 10))

        self.lbl_num_tests_gt = ttk.Label(global_test_frame, text="")
        self.lbl_num_tests_gt.pack(anchor="w", padx=5)
        ttk.Entry(global_test_frame, textvariable=self.num_teste_mtc_var, justify=tk.CENTER).pack(fill=tk.X, padx=5, pady=(0, 10))
        ...
        self.lbl_warning1 = ttk.Label(warning_frame, text="", wraplength=380, justify=tk.LEFT) # Wraplength para quebrar linha
        ...
        # --- PAINEL DIREITO ---
        self.right_panel = ttk.Frame(main_container) # Sem relief padrão
    ...
    def update_texts(self):
        ...
        # Reset button texts if not loading
    ...
    def on_click_custom(self):
        self.button_ct.config(state=tk.DISABLED) # Desabilita
    ...
    def run_process_custom(self):
        # Chama a lógica principal do teste customizado
        try:
            results = self._generate_custom_test_logic() # Separado para clareza
    ...
    def run_process_global(self):
        # Chama a lógica principal do teste global
        try:
    ...
    # --- Core Logic Methods (Should call MetaLearning class) ---
    ...
    def _generate_custom_test_logic(self):
        ...
        else: focus = 3 # Minimum temperature
        ...
        # Retorna a tupla completa de resultados
    ...
    def _generate_global_test_logic(self):
        ...
        window_type = 'Yes' # Ou ler de algum controle se necessário

        # Coleta quais modelos foram selecionados
        ...
        # Instancia a classe de lógica
        meta = MetaLearning()
        # Chama o método de lógica (Pode precisar ajustar combine_meta_learning para aceitar selected_models)
        all_models, model_ranking = meta.combine_meta_learning(
            target_variable, 0, 0, num_tests, window_type, selected_models # Passando modelos selecionados
        )
    ...
    def _display_custom_results(self, results):
        ...
        # Frame para resultados textuais
        ...
        errors = [
            ...
            (i18n.get('r2_label'), base_r2, None, meta_r2) # R2 geralmente não se aplica a Triangulação simples
        ]
        ...
        # Gráfico Matplotlib
        figure = Figure(figsize=(10, 6), dpi=100) # Ajuste o tamanho
    ...
    def _display_global_results(self, results):
        ...
        # Frame para a Tabela de Todos os Modelos
        ...
        # Ajustar colunas se necessário: all_models_table.set_column_widths([...])

        # --- Ranking e Gráfico (Colocar lado a lado ou um abaixo do outro?) ---
        ...
        ranking_table = Sheet(ranking_frame, data=ranking_data, headers=ranking_headers, width=250) # Largura fixa
        ranking_table.enable_bindings()
        ranking_table.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        # ranking_table.set_column_widths([150, 80])

        # Frame para o Gráfico de Ranking
        ...
        # Pega dados para o gráfico (Top 15?)
        top_n = 15
        ...
        figure = Figure(figsize=(8, 3), dpi=100) # Tamanho menor
        plot = figure.add_subplot(111)
        plot.bar(x_labels, y_values)
        plot.set_ylabel(i18n.get('perc_error_header'))
        # plot.set_xlabel('Models') # Pode ficar poluído, talvez remover
        plot.tick_params(axis='x', rotation=45, labelsize=8) # Rotaciona labels do eixo X
        plot.grid(True, axis='y') # Grid apenas no eixo Y
        figure.tight_layout()
